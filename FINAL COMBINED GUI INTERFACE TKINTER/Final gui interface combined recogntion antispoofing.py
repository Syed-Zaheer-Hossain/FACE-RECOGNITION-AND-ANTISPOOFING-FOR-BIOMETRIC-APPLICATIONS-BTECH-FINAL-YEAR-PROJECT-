# -*- coding: utf-8 -*-
"""
Created on Thu May 16 00:33:16 2024
...
def predict_app():
    ...
    if(rftype == 1):
        ...
        # file paths
        features_path ="E:/MACHINE LEARNING/special 3 dataset/facenet_train_features.csv"
        labels_path = "E:/MACHINE LEARNING/special 3 dataset/facenet_train_label.csv"
        ...
        yhat_test_svm_prob = svm_model.predict_proba(testX)
        
        # Check confidence and assign label if confidence is more than 70%
        confidence_threshold = 0.70
        predicted_label = None
        max_confidence = max(yhat_test_svm_prob[0])
        
        if max_confidence > confidence_threshold:
            predicted_label = out_encoder.inverse_transform([yhat_test_svm[0]])[0]
            print(f"Predicted label: {predicted_label} with confidence: {max_confidence}")
        else:
            predicted_label = 0
            # predicted_label = out_encoder.inverse_transform([yhat_test_svm[0]])[0]
            print(f"No prediction made with confidence above {max_confidence*100}%")
